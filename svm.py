...
def preprocess(Data, vec):
    ...
    vec.set_params(max_features=len(vec.vocabulary_))
    pickle.dump(vec, open("Vectorizer.pickel", "wb"))
...
Text = fileData[:-3]
testText = fileData[-3:]
dataSize = fileData.shape[0]

training, trainingLabels = preprocess(Text, vec)

testing, testingLabels = testprocess(testText)

train_x, test_x, train_y, test_y = train_test_split(training, trainingLabels)

svc = sv.fit(train_x, train_y)
    
print()
yes = 0
for test in range(test_x.shape[0]):
    res = int(svc.predict(test_x[test]))
    print(res, test_y[test], test)
    if res == test_y[test]:
        yes = yes + 1
    
for test in range(testing.shape[0]):
    res = int(svc.predict(testing[test]))
    print(res, testing[test,1], test)
    if res == testing[test,1]:
        yes = yes + 1

print(f"Accuracy is: {yes/dataSize*100}%")